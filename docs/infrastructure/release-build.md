...
**Track _name_, however, does need to be right, and it's not what you'd guess.** Per [Google's Play Developer API docs](https://developers.google.com/android-publisher/tracks) and [Play Console help](https://support.google.com/googleplay/android-developer/answer/13295490?hl=en), Wear OS releases (rolled out as "dedicated form factor release tracks" since March 2023) live on separate track IDs computed as `[prefix]:defaultTrackName` -- Wear's prefix is `wear`, and, non-obviously, **Internal Testing's default track name is `qa`, not `internal`**. So the Wear deploy step above uses `tracks: wear:qa`, not `wear:internal`. This is sourced from Google's docs but not yet verified against this app's actual track list -- if it's wrong, the action fails with a clear "Track(s) could not be found. Available tracks are: ..." error that lists the real names, which is the fastest way to confirm or correct it. Worth checking first when we test this end-to-end.

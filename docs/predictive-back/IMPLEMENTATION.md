...
`android:enableOnBackInvokedCallback="true"` must be present on `<application>` for any of this to activate. It can't be injected by the plugin -- `update_android_manifest()` only inserts new child elements before a closing tag, with no path to an attribute on the tag itself. Instead, `apps/threshold/src-tauri/build.rs` patches it in directly (idempotently, on every Android build), the same way `usesCleartextTraffic` is already app-shell-level config in this generated project, not plugin-level.

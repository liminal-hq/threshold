...
This ordering is deliberate, not an oversight: durable-persist-first means a process death between the two steps still leaves Rust with a record that the alarm fired (degraded to "no fast native ring", today's pre-Phase-3 behaviour), rather than losing the fire entirely. The unavoidable consequence is that `persist` runs _before_ this function knows what `NativeEventBus.publish()` is even going to return, so the payload handed to `persist` -- and therefore everything downstream of it, including `AlarmFired.handled_natively` as Rust ultimately sees it — always carries an **empty** `handledNatively`. The `"watch-ring"` tag genuinely exists and is exercised by unit tests (`should_skip_native_watch_ring`'s Kotlin and Rust suites both cover the tag-present case directly), but on the one call path that actually runs in production today, it is structurally never populated. `send_alarm_ring`'s protection against a redundant ring on a stale-but-otherwise-normal replay comes from the staleness check alone, backstopped by the watch's own ring de-duplication (`WearRingingService` ignores a duplicate ring message for an alarm it's already ringing — see [wear-sync.md](../plugins/wear-sync.md)) rather than from the tag. This is a known, accepted limitation of prioritising durability over the best-effort dedup hint on this synchronous path, not a bug to fix quietly. Note that `send_alarm_ring` is only reached at all once `alarm:fired` has been emitted -- issue #314's coordinator-side guard (below) can suppress the fire before it gets this far, which is a materially different and stronger protection than staleness alone for the case it covers.
...
5. Independently, whenever Rust does boot (already running, or catching up from the durable queue), the app crate's `alarm-manager:native-fired` listener (`lib.rs`) deserialises the payload, checks `EventDedup`, and calls `AlarmCoordinator::report_alarm_fired`, which -- since issue #314 -- first checks whether the alarm's persisted `next_trigger` shows this occurrence has already been superseded by a dismiss or snooze (`is_superseded_fire`, `apps/threshold/src-tauri/src/alarm/mod.rs`) before emitting the canonical `alarm:fired` event app-wide; a superseded fire is dropped with a log line instead.
...
**Issue #314 -- the alarm-manager/wear-sync pipeline-ready race:** `alarm-manager` and `wear-sync` each gate their own `DurableEventQueue` drain behind an independent "pipeline ready" signal (`markAlarmPipelineReady`/`markWatchPipelineReady`), with no ordering guarantee between them. On a cold boot triggered by a watch-originated dismiss (rather than the alarm's own full-screen intent), wear-sync's gate has been observed firing first -- its queued dismiss replays into Rust and commits -- with alarm-manager's gate firing roughly a second or two later, only then draining the _original_ queued fired event for the same alarm. Before this fix, that stale fired event reached `report_alarm_fired` unconditionally and re-rang the watch after the user had already dismissed it. `is_superseded_fire` closes this at the one place both racing paths converge (`AlarmCoordinator`, which owns alarm state) rather than by trying to reorder or couple the two independent gates -- see the function's doc comment for why a `next_trigger`-shape check, not a time-window debounce, is what actually distinguishes a live fire from a stale one here.

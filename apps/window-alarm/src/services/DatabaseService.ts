  ...
  async saveAlarm(alarm: Omit<Alarm, 'id'> & { id?: number }): Promise<number> {
    ...
    if (alarm.id) {
      await this.db!.execute(
        ...
        [alarm.label, alarm.enabled, alarm.mode, alarm.fixedTime, alarm.windowStart, alarm.windowEnd, daysJson, alarm.nextTrigger, alarm.id]
      );
      return alarm.id;
    } else {
      const result = await this.db!.execute(
        `INSERT INTO alarms (label, enabled, mode, fixed_time, window_start, window_end, active_days, next_trigger) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [alarm.label, alarm.enabled, alarm.mode, alarm.fixedTime, alarm.windowStart, alarm.windowEnd, daysJson, alarm.nextTrigger]
      );

...
function ArtifactRow({ label, path, sig }: { label: string; path: string; sig: string | null }) {
	...
	return (
		<Text>
			...
			{label.padEnd(12)}
			{size.padEnd(10)}
			{'release/' + filename.padEnd(30)}
			{pipe}{' '}
			{sig ? (
				<Text>
					<Text color={palette.bar}>{'\u2713'}</Text> {sig}
				</Text>
			) : (
				''
			)}
	...
	return (
		...
		>
			...
			{!isRedo && filesChanged > 0 && (
				<Text>
					{' '}
					<Text color={palette.bar}>{'\u2713'}</Text> Updated {filesChanged} files
				</Text>
			)}
			{hasArtifacts ? (
				<Text>
					{' '}
					<Text color={palette.bar}>{'\u2713'}</Text> Created commit:{' '}
					<Text color={palette.text}>{commitMsg}</Text>
				</Text>
			) : (
				<>
					<Text>
			...
			{isRedo ? (
				<Text>
					{' '}
					<Text color={palette.bar}>{'\u2713'}</Text> Updated tag{' '}
					<Text color={palette.yellow}>{tagName}</Text> {'\u2192'} HEAD
				</Text>
			) : (
				<Text>
					{' '}
					<Text color={palette.bar}>{'\u2713'}</Text> Tagged{' '}
				...
				<>
					...
					<Box>
						<Text>
							...
							{''.padEnd(45)}
							<Text color={palette.line}>|</Text> <Text bold>Signatures</Text>
						</Text>
					</Box>
					<Text>
						{' '}
						{'-'.repeat(52)} <Text color={palette.line}>|</Text> {'-'.repeat(22)}
					</Text>
					{artifacts.phone?.aab && (
						<ArtifactRow
							label="Phone AAB"
					...
					{artifacts.phone?.symbols && (
						<ArtifactRow label="Phone syms" path={artifacts.phone.symbols} sig={null} />
					)}
					{artifacts.wear?.aab && (
						<ArtifactRow
							label="Wear AAB"
					...
					{artifacts.wear?.mapping && (
						<ArtifactRow label="Wear map" path={artifacts.wear.mapping} sig={null} />
					)}
				</>
			)}
			...
			{isRedo || forceTag ? (
				<Text>
					{' '}
					<Text color={palette.accent}>Next</Text> git push origin HEAD && git push origin {tagName}{' '}
					--force
				</Text>
			) : (
				<Text>
			...
			{hasArtifacts && (
				<>
					<Text>
						{' '}
						<Text color={palette.accent}>Play</Text> https://play.google.com/console (AABs)
					</Text>
					<Text>
						{' '}
						<Text color={palette.accent}>GitHub</Text> Upload APKs to GitHub release (APKs)
					</Text>
				</>
			)}
			{!hasArtifacts && !isRedo && (
				<Text>
					{' '}
					<Text color={palette.accent}>Build</Text> pnpm build:release
				</Text>
			)}
			{isRedo || forceTag ? (
				<>
					<Text>
						{' '}
						<Text color={palette.red}>Undo</Text> git reset --soft HEAD~1
					</Text>
					<Text> </Text>
				...
				<Text>
					{' '}
					<Text color={palette.red}>Undo</Text> git reset --soft HEAD~1 && git tag -d {tagName}
				</Text>
			)}
		</Layout>
	);

...
function redrawLastScreen() {
	...
	drawScreen(
		TERMINAL.lastLines,
		Array.isArray(TERMINAL.lastFooterLines) ? TERMINAL.lastFooterLines : [],
	);
}
...
function renderTwoColumnPanels(leftTitle, leftRows, rightTitle, rightRows, totalWidth) {
	...
	for (let i = 0; i < maxRows; i += 1) {
		...
		lines.push(
			`${padRight(truncate(left, colWidth), colWidth)}${' '.repeat(gap)}${truncate(right, colWidth)}`,
		);
	}
...
async function askWithChoices(rl, question, defaultValue, choices, helpText = '') {
	...
	while (true) {
		...
		const answer = (
			await rl.question(`${paint('?', COLOUR.cyan)} ${question} [${defaultValue}]: `)
		).trim();
...
function writeCyclingPrompt(question, cycleChoices, selectedIndex) {
	const options = cycleChoices
		.map((item, index) =>
			index === selectedIndex ? paint(`[${item.label}]`, COLOUR.cyan) : item.label,
		)
		.join('  ');
	output.write(
...
function updateWearGradle(gradleText, versionName, versionCode) {
	...
	outputText = outputText.replace(/versionName\s*=\s*"[^"]+"/, `versionName = "${versionName}"`);
	outputText = outputText.replace(/versionCode\s*=\s*\d+/, `versionCode = ${versionCode}`);
...
function renderStatusBoard(
	...
	const currentRows = [
		...
		keyValueLine(
			'Phone code override',
			String(currentState.tauriVersionCode ?? 'unset'),
			panelWidth,
		),
	...
	const draftRows = [
		...
		keyValueLine(
			...
			currentState.tauriVersionCode == null
				? 'leave unset'
				: String(
						draftState.tauriDerivedVersionCode != null ? draftState.tauriDerivedVersionCode : '-',
					),
			panelWidth,
		),
		keyValueLine(
		...
		keyValueLine(
			'Wear code',
			String(draftState.wearVersionCode != null ? draftState.wearVersionCode : '-'),
			panelWidth,
		),
		keyValueLine('Tag', String(draftState.tagName ?? '-'), panelWidth),
		keyValueLine(
			'Update web version',
			draftState.updateWebVersion == null ? '-' : draftState.updateWebVersion ? 'yes' : 'no',
			panelWidth,
		),
	];
	...
	if (currentState.tauriVersionName !== currentState.webVersion) {
		anchoredBottomLines.push(
			paint('Warning: Tauri and web package versions are out of sync.', COLOUR.yellow),
		);
	}
...
function renderRecentTags(tags) {
	...
	for (const tag of tags) {
		...
		console.log(
			`- ${paint(tag.name, COLOUR.green)}  ${paint(`(version ${version})`, COLOUR.grey)}  ${paint(tag.createdAt, COLOUR.grey)}`,
		);
	}
}
...
async function chooseVersionName(rl, currentState, draftState) {
	while (true) {
		renderStatusBoard(currentState, draftState, '', [
		...
		if (choice === '4' || choice === 'c') {
			const candidate = await ask(
				rl,
				'Enter custom semantic version',
				currentState.tauriVersionName,
			);
...
async function maybeUpdateOrCreateTag(
	rl,
	currentState,
	draftState,
	tagName,
	...
	if (updateExistingLocalTag) {
		...
		if (tagExistsOnOrigin(tagName)) {
			console.log(
				paint(
					`Remote ${tagName} still points to previous commit until manual force-push.`,
					COLOUR.yellow,
				),
			);
		}
...
async function main() {
	...
	try {
		while (true) {
			...
			renderStatusBoard(currentState, draftState, 'Planned updates', [
			...
			if (nextState.autoCommitAndTag) {
				...
				console.log(
					'- Commit + tag order: apply file updates -> commit release bump -> create/update local tag',
				);
			} else {
			...
			if (unrelatedChanges.length > 0) {
				...
				if (unrelatedChanges.length > sample.length) {
					console.log(
						`  - ${paint(`...and ${unrelatedChanges.length - sample.length} more`, COLOUR.yellow)}`,
					);
				}
			}
			console.log(divider());
			...
			if (unrelatedChanges.length > 0) {
				renderStatusBoard(
					...
					[
						paint(
							'Applying release updates can be done, but confirm before proceeding.',
							COLOUR.yellow,
						),
					],
					[
			...
			if (changedFiles.length > 0) {
				console.log(
					paint(
						`Version updates applied (${changedFiles.length} file${changedFiles.length === 1 ? '' : 's'}).`,
						COLOUR.green,
					),
				);
			} else {
				console.log(
					paint(
						'No version file changes were needed; continuing with tag workflow.',
						COLOUR.yellow,
					),
				);
			}
			...
			if (nextState.autoCommitAndTag) {
				...
				if (nextState.updateExistingLocalTag) {
					console.log(
						paint(
							`Updated local tag ${nextState.tagName} to the new release commit.`,
							COLOUR.green,
						),
					);
				} else {
					console.log(
						paint(
							`Created local tag ${nextState.tagName} on the new release commit.`,
							COLOUR.green,
						),
					);
				}
				if (tagExistsOnOrigin(nextState.tagName)) {
					console.log(
						paint(
							`Remote ${nextState.tagName} still points to the previous commit until manual force-push.`,
							COLOUR.yellow,
						),
					);
				}
			...
			if (nextState.autoCommitAndTag) {
				...
				console.log(
					`- Push branch and tag manually when ready: git push origin HEAD && git push origin ${nextState.tagName}`,
				);
			} else {
